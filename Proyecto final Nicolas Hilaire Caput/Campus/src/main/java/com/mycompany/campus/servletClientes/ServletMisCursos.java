 ...
 * @author WEB 2
 */
@WebServlet(name = "ServletMisCursos", urlPatterns = {"/ServletMisCursos"})
public class ServletMisCursos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("UserCur",
                crud.misCursos(((Cliente) session.getAttribute("logClien")).getId())
        ); 
        request.setAttribute("UserCurNop",
                crud.misCursosNop(((Cliente) session.getAttribute("logClien")).getId())
        ); 
        
        RequestDispatcher rs = request.getRequestDispatcher("/WEB-INF/Layout.jsp");

        request.setAttribute("pagina", "IndexUser");
